...
// File: @phcfinance/matrixswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/PHCFianceProfile.sol

pragma solidity ^0.6.0;

/** @title PHCFianceProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract PHCFianceProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public phcfToken;
    ...
    uint256 public numberPhcfToReactivate;
    uint256 public numberPhcfToRegister;
    uint256 public numberPhcfToUpdate;
    ...
    constructor(
        IBEP20 _phcfToken,
        uint256 _numberPhcfToReactivate,
        uint256 _numberPhcfToRegister,
        uint256 _numberPhcfToUpdate
    ) public {
        phcfToken = _phcfToken;
        numberPhcfToReactivate = _numberPhcfToReactivate;
        numberPhcfToRegister = _numberPhcfToRegister;
        numberPhcfToUpdate = _numberPhcfToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends PHCF to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer PHCF tokens to this contract
        phcfToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberPhcfToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer PHCF token to this address
        phcfToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberPhcfToUpdate
    ...
    /**
     * @dev Claim PHCF to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        phcfToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of PHCF to register
     * Callable only by owner admins.
     */
    function updateNumberPhcf(
        uint256 newNumberPhcfToReactivate,
        uint256 _newNumberPhcfToRegister,
        uint256 _newNumberPhcfToUpdate
    ) external onlyOwner {
        numberPhcfToReactivate = newNumberPhcfToReactivate;
        numberPhcfToRegister = _newNumberPhcfToRegister;
        numberPhcfToUpdate = _newNumberPhcfToUpdate;
    }
